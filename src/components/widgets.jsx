...
import { Card, CardContent, Typography, Grid, Box, Button } from "@mui/material";  // MUI components
...
const Widgets = () => {
  ...
  const localSelectedDataset = JSON.parse(localStorage.getItem("selectedDataset"));
  ...
  <Card>
            <CardContent>
              ...
              {isStudyLoaded && selectedStudy ? (
                <div>
                  <Typography variant="body1"><strong>Study Name:</strong> {selectedStudy.name}</Typography>
                  <Typography variant="body1"><strong>Study ID:</strong> {selectedStudy.studyId}</Typography>
                  <Typography variant="body1"><strong>Description:</strong> {selectedStudy.description}</Typography>
                </div>
              ) : (
      ...
      <Box sx={{ mb: 2 }}>
        <Button
          ...
          color="secondary"
          onClick={handleBack}
          sx={{ fontSize: "0.875rem", height: "40px", marginRight: "10px", background:'red', color:'white' }}
        >
          Back
        </Button>
      </Box> 
